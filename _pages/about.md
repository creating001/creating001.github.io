---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<span class='anchor' id='about-me'></span>

我是吉林大学计算机科学与技术专业的一名本科生。我的学术兴趣聚焦于人工智能与竞赛编程的交叉领域，具备扎实的算法、数据结构和深度学习基础。目前，我正致力于多模态大语言模型的研究，重点关注Qwen2.5-VL和Qwen2.5-Omni等先进模型。我深入研究多模态对齐、跨模态表征学习以及模型架构设计，致力于通过严谨的算法设计与创新的模型构建，推动多大模型能力边界的不断拓展。


# 📖 Educations

- B.S. in Computer Science and Techonolgy, Jilin University

  *2022.09 - 2026.06*

  GPA: 3.9/4.0 (Rank: 1/350)

# 🎖️ Honors and Awards

- *2025.05*, ICPC国际大学生程序设计竞赛全国邀请赛(南昌)金奖
- *2025.04*, 第37次CCF计算机软件能力认证430分, 全国前0.11%
- *2024.12*, 本科生国家奖学金
- *2024.11*, ICPC国际大学生程序设计竞赛亚洲区域赛(南京)银奖
- *2024.11*, 解放领航奖学金(一汽解放汽车有限公司)
- *2024.10*, CCF大学生计算机系统与程序设计竞赛国家一等奖
- *2023.12*, 本科生国家奖学金
